        ...
        <div className="bg-background border border-border rounded-lg shadow-lg p-3">
          ...
          {payload.map((entry: any, index: number) => (
            <p key={index} className="text-sm" style={{ color: entry.color }}>
              {entry.name}: {entry.name.includes("Hours") || entry.name.includes("Duration")
                ? `${formatTime(entry.value)} (${entry.value.toFixed(2)}h)`
                : entry.value}
            </p>
          ))}
        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{formatTime(statistics.total_duration_hours)}</div>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {statistics.total_recordings > 0
                        ? formatTime(statistics.total_duration_hours / statistics.total_recordings)
                        : "00:00:00"}
                    </div>
                        ...
                        {statistics.by_speaker.map((speaker) => (
                          <div key={speaker.speaker_id} className="flex justify-between items-center p-3 border rounded-lg">
                            ...
                            <div className="text-right">
                              <p className="font-semibold">{formatTime(speaker.duration_hours)}</p>
                      ...
                      <div className="space-y-2">
                        {statistics.by_book.map((book) => (
                          <div key={book.book_id} className="flex justify-between items-center p-3 border rounded-lg">
                            ...
                            <div className="text-right">
                              <p className="font-semibold">{formatTime(book.duration_hours)}</p>
                      ...
                      <div className="space-y-2">
                        {statistics.by_category.map((category) => (
                          <div key={category.category_id} className="flex justify-between items-center p-3 border rounded-lg">
                            ...
                            <div className="text-right">
                              <p className="font-semibold">{formatTime(category.duration_hours)}</p>
